...
방어선이 전부 거기 있습니다. 테스트넷에서 `--dry`로 시작하세요.
...
발급하세요. 에이전트 키는 **거래만 가능하고 출금은 불가능**하며 언제든 폐기할 수
있습니다. 읽기 전용 도구는 키 없이도 동작합니다.
...
- 에이전트 키는 거래 전용, 이 소프트웨어는 자금을 밖으로 옮길 수 없습니다
...
(이 시장 실측 승률 천장 약 58%) 국면이 바뀌면 사라질 수 있습니다.
무엇을 하는지 정확히 이해할 때까지 테스트넷에서 돌리고, 잃어도 되는 돈만
거세요.
