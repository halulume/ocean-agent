...
def review_and_adapt(client: PacificaClient, policy: dict, st: dict) -> None:
    ...
    # 줄여야 하고(승률이 높아도 합계가 마이너스면 축소), 승률 낮아도 순이익이면 유지.
...
MODE_PRESETS = {
    "aggressive": {   # 테스트넷 학습용: 많이 거래 = 데이터 폭증
        # 문턱을 높게 잡으면 통과 조합이 사라져 거래가 멈춘다. 바닥은
        # 순수엣지 관문이 따로 지키므로 여기서는 낮게 둔다.
        "min_win_rate": 0.45,
    ...
    "careful": {      # 메인넷 실전용: 안 잃는 게 최우선
        # 너무 높게 잡으면 통과 조합이 0개가 되어 careful 로 바꾸는 순간
        # 거래가 완전히 멈춘다. '보수적'이 아니라 '고장'이다. 도달 가능한
        # 범위에서 aggressive 보다만 높게 잡는다.
